...
export class TransactionBuilderABI {
  ...
  constructor(abis: Bytes[], builderArg: ABIBuilderConfig) {
    abis.forEach((abi) => {
  ...
  static serializeArg(argVal: any, argType: TypeTag, serializer: Serializer) {
  ...
  static toBCSArgs(abiArgs: any[], args: any[]): Bytes[] {
  ...
  static argToTransactionArgument(argVal: any, argType: TypeTag): TransactionArgument {
    if (argType instanceof TypeTagBool) {
  ...
  static toTransactionArgument(abiArgs: any[], args: any[]): TransactionArgument[] {
    if (abiArgs.length !== args.length) {
      throw new Error('Wrong number of args provided.');
    }
